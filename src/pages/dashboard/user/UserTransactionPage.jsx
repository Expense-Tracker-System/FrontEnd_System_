                ...
                <p className='text-3xl font-bold text-center'>RS:{getTotalAmount().toFixed(2)}</p>
                            ...
                            <div className='flex flex-row w-full p-5 justify-between'>
                                ...
                                <div className='rounded shadow-lg border p-5'>
                                    ...
                                    <form onSubmit={(e) => { e.preventDefault(); addIncomeToAPI(); }} className='flex text-center flex-col mx-auto border-double border-indigo-50'>
                                        ...
                                        <button className='bg-emerald-900 hover:bg-violet-600 active:bg-violet-700 focus:outline-none text-white px-4 py-2 rounded-md'>{editId ? 'Update Income' : 'Add Income'}</button>
                                    </form>
                                </div>
                                <div className='rounded shadow-lg border p-5'>
                                    ...
                                    <form onSubmit={(e) => { e.preventDefault(); addExpenseToAPI(); }} className='flex text-center flex-col mx-auto border-double border-indigo-50'>
                                        ...
                                        <button className='bg-emerald-900 hover:bg-violet-600 active:bg-violet-700 focus:outline-none text-white px-4 py-2 rounded-md'>{editId ? 'Update Expense' : 'Add Expense'}</button>
                                    </form>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className='mt-10 ml-4'>
                            <table className='min-w-full bg-white'>
                                <thead>
                                    <tr>
                                        <th className='py-2 px-11'>Description</th>
                                        <th className='py-2 px-11'>Amount</th>
                                        <th className='py-2 px-11'>Type</th>
                                        <th className='py- px-11'>Actions</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {transactions.map(transaction => (
                                        <tr key={transaction.id}>
                                            <td className='border px-11 py-2'>{transaction.description}</td>
                                            <td className={`border px-11 py-2 ${transaction.amount < 0 ? 'text-red-500' : 'text-green-500'}`}>
                                                {transaction.amount < 0 }{Math.abs(transaction.amount).toFixed(2)}
                                            </td>
                                            <td className='border px-4 py-2'>{transaction.amount < 0 ? 'Expense' : 'Income'}</td>
                                            <td className='border px-4 py-2'>
                                                <button
                                                    className=' bg-teal-900 hover:bg-blue-700 text-white font-bold py-1 px-4 rounded mr-2'
                                                    onClick={() => handleEdit(transaction)}
                                                >
                                                    Edit
                                                </button>
                                                <button
                                                    className=' bg-teal-900 hover:bg-red-700 text-white font-bold py-1 px-4 rounded'
